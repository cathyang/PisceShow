...
function fetchDescription(photo_id) {
	$('#descriptionSpan').load("service.php?request=getdescription&id="+photo_id+"", function(data){
		...
		$('#descriptionSpan').html(data + "<br/><a href=\"javascript:changeRez('"+medium_url+"');\">Medium Size</a> | <a href=\"javascript:changeRez('"+large_url+"');\">Large Size</a>");
...
function thumbClick(url, photo_id) {
	if($('#focusImg').attr('src')!=url)
	{
		...
		timeout = 0;
		//$('#loading').css({height:(document.body.clientHeight-155)});
		$('#loadingImg').fadeIn('slow');
		$('#loading').fadeIn('slow');
		$('#descriptionSpan').fadeOut('slow');
		$('#focusImg').fadeOut('slow', function(){
...
$(document).ready(function(){
		...
		$(document).ajaxError(function(){
			//used for debugging why our ajax calls fail...
			//alert("Error!");
		});
		...
		//alert("fetching thumbs");
		$('img.intro').click(function(e){
			$('#intro').fadeOut("slow");
...
function fetchThumbs(op, name, searchterms, page){
	...
	$('#thumbnails').load("service.php?request="+op+"&user="+name+"&tags="+searchterms+"&page="+page+"", function(text){
